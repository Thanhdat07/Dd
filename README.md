...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; }
        body { background-color: #121212; color: #ffffff; display: flex; justify-content: center; align-items: center; min-height: 100vh; }
        
        .container { background-color: #1e1e1e; padding: 30px; border-radius: 12px; box-shadow: 0 4px 20px rgba(0,0,0,0.5); width: 100%; max-width: 600px; text-align: center; }
        ...
        input, select { width: 100%; padding: 10px; border-radius: 6px; border: 1px solid #333; background-color: #2c2c2c; color: white; outline: none; }
        input:focus, select:focus { border-color: #00e676; }
        
        button { background-color: #00e676; color: #121212; font-weight: bold; padding: 12px 20px; border: none; border-radius: 6px; cursor: pointer; width: 100%; font-size: 1.1rem; transition: 0.3s; margin-top: 10px; }
        ...
        .logout-btn { background-color: #ff5252; width: auto; padding: 8px 15px; font-size: 0.9rem; margin: 0; color: white; }
        
        /* Dashboard Sections */
        .card { background-color: #2c2c2c; padding: 20px; border-radius: 10px; margin-bottom: 20px; border: 1px solid #444; }
        ...
        .btn-attendance:disabled { background-color: #555; cursor: not-allowed; color: #888; }

        /* Đớp */
        .mouth-btn { font-size: 5rem; cursor: pointer; user-select: none; transition: transform 0.1s; display: inline-block; background: none; border: none; outline: none; margin: 10px 0; }
        .mouth-btn:active { transform: scale(0.85); }
        .click-count { font-size: 1.2rem; color: #ff4081; margin-top: 10px; }
        ...
        .rank { font-weight: bold; color: #00e676; width: 30px; }
        
        .error { color: #ff5252; margin-top: 10px; display: none; font-weight: bold;}
    </style>
</head>
<body>
    ...
    <div class="main-container" id="main-screen">
        <div class="header">
            <div class="user-info">
                ...
                <small id="display-role">Chức vụ</small>
            </div>
            <div>
        ...
        <div class="card">
            ...
            <p>Nhấp vào miệng để húp 1,000 VND</p>
...
<script>
    ...
    function handleAuth() {
        ...
        if(isRegisterMode) {
            if(db.users[user]) return showError("Tên người dùng đã tồn tại! Vui lòng chọn tên khác.");
            ...
            db.users[user] = {
                ...
                balance: 0,
                clicks: 0,
                streak: 0, // Số ngày điểm danh liên tiếp (tối đa 7)
                lastActiveDate: getTodayStr(), // Dùng để tính ngày bị lỡ điểm danh
                lastAttendanceDate: ""
            };
            saveDB();
            ...
            // Xử lý phạt không điểm danh khi đăng nhập
    ...
    function applyPenaltyIfNeeded() {
        ...
        if (userData.lastActiveDate && userData.lastActiveDate !== today) {
            ...
            if (daysMissed > 1) {
                // Trừ ngày hiện tại ra, những ngày ở giữa là ngày không vào web
                let actualMissed = daysMissed - 1;
                let penalty = actualMissed * 500000;
                userData.balance -= penalty;
                userData.streak = 0; // Reset chuỗi điểm danh
    ...
    function updateUI() {
        ...
        if(userData.lastAttendanceDate === today) {
            ...
            attStatus.innerText = "Bạn chưa điểm danh hôm nay!";
        }
    }
    ...
    function markAttendance() {
        ...
        if(userData.lastAttendanceDate === today) return; // Đã điểm danh
        
        // Tính tiền thưởng
        userData.streak = (userData.streak % 7) + 1; // Từ 1 tới 7, sau 7 quay về 1
        let reward = 1000000 * Math.pow(2, userData.streak - 1); // 1M, 2M, 4M, 8M...
    ...
    function dop() {
        ...
        // Cập nhật DOM nhanh không cần load lại toàn bộ UI
        document.getElementById('display-balance').innerText = formatMoney(userData.balance);
        document.getElementById('display-clicks').innerText = userData.clicks.toLocaleString('vi-VN');
    ...
    function updateLeaderboard() {
        ...
        // Chuyển Object users thành Array để sort
        ...
        // Sắp xếp theo số lượt click giảm dần
            ...
            listStr += `
                <div class="leaderboard-item">
                    <div>
                        ...
                        <small style="color: #aaa;">(${u.role})</small>
                    </div>
                    <div style="color: #ff4081; font-weight: bold;">
